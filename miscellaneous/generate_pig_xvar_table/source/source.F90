   ...
   if(mgrid%rankl==0) then
    ...
    do j=0,ddx2-1
     do i=0,ddx1-1
       ...
       open(unit=fp,file=filename,status='old', &
    ...
    do j=1,NT+1
     do i=1,Nrho+1
      write(u) & 
      real(table_glob(i,j)%var(id_f),kind=8), &
      real(table_glob(i,j)%var(id_dfdrho),kind=8), &
      real(table_glob(i,j)%var(id_dfdT),kind=8), &
      real(table_glob(i,j)%var(id_d2fdrho2),kind=8), &
      real(table_glob(i,j)%var(id_d2fdT2),kind=8), &
      real(table_glob(i,j)%var(id_d2fdrhodT),kind=8), &
      real(table_glob(i,j)%var(id_d3fdrho2dT),kind=8), &
      real(table_glob(i,j)%var(id_d3fdrhodT2),kind=8), &
      real(table_glob(i,j)%var(id_d4fdrho2dT2),kind=8)
     end do
    end do
    ...
    do j=1,NT+1
     do i=1,Nrho+1
      write(u) &
      real(table_glob(i,j)%var(id_dPdrho),kind=8), &
      real(table_glob(i,j)%var(id_dcdrho),kind=8), &
      real(table_glob(i,j)%var(id_dcdT),kind=8), &
      real(table_glob(i,j)%var(id_d2cdrhodT),kind=8)
     end do
    end do
