...
### 8. `Udo`: User-defined output quantities

`Udo` reads all `udo_XXXXX.dat` files containing user-defined scalar quantities. These quantities are defined in `app.F90` (in the `extract_userdef_quantities` subroutine) and are computed after each time step. The output files are stored in the `/udos` directory.
...
To extract the time series of a given quantity (for example, the fourth variable):

```python
divb_rel = udo.time_series(4)
```

In this example, `divb_rel` contains the time evolution of the mean relative magnetic field divergence.

The variable index passed to `time_series()` follows the same ordering as the quantities defined in the `extract_userdef_quantities` subroutine in `app.F90`.

### 9. `h5rays`: ray output reader

The `h5rays` class reads `rays_XXXXX.h5` files stored in the `/rays` directory. Ray outputs are generated by subsampling the computational grid along the radial and angular directions (see the `SAVE_RAYS` option). Each ray file contains the primitive variables, temperature, Cartesian coordinates, and (when MHD is enabled) the magnetic field.

Initialize the reader as

```python
r = h5rays(-1, path='./rays/')
```

By default, the first argument is interpreted as the index of the sorted list of ray files (`mode='i'`). Thus `-1` loads the latest available output, while `0` loads the first one.

Alternatively, a file can be loaded by its output number using

```python
r = h5rays(42, path='./rays/', mode='n')
```

which opens `rays_n00042.h5`.

The simulation time and output metadata are available as attributes:
...
Physical quantities can be extracted using
...
radii = r.r()      # spherical radius
xyz   = r.coords() # Cartesian coordinates

vel = r.vel()      # velocity vector
vr  = r.vr()       # radial velocity
vh  = r.vh()       # tangential velocity
```

For MHD simulations, the magnetic field can be accessed as

```python
B  = r.bfield()    # magnetic field vector
Br = r.br()        # radial component
